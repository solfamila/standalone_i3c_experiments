...
EXPERIMENTS := master_polling master_interrupt master_dma_irq_probe master_i3c_dma_irq_probe master_i3c_dma_byte_bridge master_i3c_dma_seed_chain_probe master_i3c_dma_seed_tail_ibi_probe master_i3c_sdma_seed_tail_len_sweep master_led_smoke
...
help:
	@printf '%s\n' \
	  ...
	  '  make master_i3c_sdma_seed_tail_len_sweep Build, flash, run, and validate the segmented seed-tail length sweep' \
	  ...
	  '  make clean-master_i3c_sdma_seed_tail_len_sweep Remove only master_i3c_sdma_seed_tail_len_sweep/_build' \
